...
将经验、错误和修正记录到 `.agents/learnings/` 中，持续改进 AI 辅助质量。
重要经验最终晋升到 `.agents/instructions.md`，使每次对话都能从历史中学习。
...
| 场景 | 操作 |
|------|------|
| 命令/工具执行失败 | 记录到 `.agents/learnings/ERRORS.md` |
| 用户纠正 Agent | 记录到 `.agents/learnings/LEARNINGS.md`，分类 `correction` |
| 用户想要新功能 | 记录到 `.agents/learnings/FEATURE_REQUESTS.md` |
| 外部 API/工具失败 | 记录到 `.agents/learnings/ERRORS.md`，附集成细节 |
| 知识过时/不准确 | 记录到 `.agents/learnings/LEARNINGS.md`，分类 `knowledge_gap` |
| 发现更好的方案 | 记录到 `.agents/learnings/LEARNINGS.md`，分类 `best_practice` |
| 广泛适用的经验 | 晋升到 `.agents/instructions.md` |
...
当一条经验**广泛适用**时，将其晋升到 `.agents/instructions.md`：

1. 将经验提炼为简洁规则
2. 追加到 `.agents/instructions.md` 的对应章节
...
```bash
grep -rh "状态\*\*: pending" .agents/learnings/*.md | wc -l
```
