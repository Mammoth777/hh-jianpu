...
### Fixed
- 🔧 **低音点与歌词重叠** - 修复连音组中低八度音符的低音点（如 `6'//`）与下方歌词文字产生重叠的问题
- 🔧 **升号音符 beam 组划分错误** - 修复 `#音符/` 前有空格时因空格信息记录在 SHARP token 上导致与相邻 beam 组错误连接的问题
...
### Added
- ✏️ **编辑器↔预览滚动同步** - 编辑器滚动时右侧曲谱预览自动跟随滚动到对应小节
- 📏 **源文本换行渲染** - 编辑区换行后渲染区自动跟随换行（不再固定每行 4 小节）

### Changed
- 🎨 **合并编辑/演奏模式** - 移除模式切换按钮，编辑器和演奏控制栏合并为统一界面
- 🔇 **编辑时禁用播放控制** - 编辑器获得焦点时播放控制栏自动禁用，避免空格键冲突

### Fixed
- 🔧 **渲染区行分行** - 修复编辑区换行后渲染区不换行的问题（sourceLine 始终为 1 的 bug）
